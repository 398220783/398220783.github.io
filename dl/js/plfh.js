...
$('#start').click(function(){
  ...
  arr.forEach((item,index)=>{
    ...
    $.ajax({
      type:'get',
      url:`//wx.urlfh.com/dwz.php?dwzapi=${$('#shorttype').attr('data-value')}&type=${$('#jiekou').attr('data-value')}&longurl=${item}`,
      success:function(data){
        if(JSON.parse(data).code==1){
          $('#success').text(parseInt($('#success').text())+1)
          successarr.push({
            item: JSON.parse(data).ae_url,
            index: index
          })
          $(`#w${index}`).html(`<span style="color:green">${JSON.parse(data).ae_url}</span>`)
        }else{
          ...
          $(`#w${index}`).html(`<span style="color:red">${JSON.parse(data).msg}</span>`)
        }
        end(arr.length)
      },
      error:function(){
